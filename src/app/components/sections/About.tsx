...
export function About() {
  ...
  return (
    <section id="sobre" className="border-t border-border py-16 sm:py-24 lg:py-32">
      <Container>
        ...
        <div className="mt-10 grid grid-cols-1 gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <h2 className="font-display text-[2.25rem] leading-[1.05] tracking-[-0.02em] sm:text-[3rem]">
              Não vendemos carros.
              <br />
              <span className="text-white/38">Entregamos exceções.</span>
            </h2>
            <div className="mt-10 flex items-start gap-6">
              <div className="shrink-0">
                <p className="font-display text-[3.5rem] leading-none tracking-tight text-accent">5%</p>
                <p className="mt-2 max-w-[10rem] text-[0.8125rem] leading-snug text-white/55">
        ...
        {/* Principles */}
        <div className="mt-16 grid grid-cols-1 gap-px overflow-hidden rounded-xl border border-border bg-white/[0.06] md:grid-cols-3">
          {principles.map((p) => (
            <div key={p.n} className="flex flex-col gap-3 bg-background p-8">
              <span className="font-mono text-[0.75rem] text-accent">{p.n}</span>
              <h3 className="font-display text-[1.25rem] tracking-tight text-white">{p.title}</h3>
              <p className="text-[0.875rem] leading-relaxed text-white/55">{p.text}</p>
            </div>
          ))}
        </div>
      </Container>
